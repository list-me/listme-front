...
export const TempModal: React.FC<ICustomModalProps> = ({
  isOpen,
  onClickModal = () => {},
}) => {
  ...
  return (
    <Container>
      ...
      >
        <SideBar>
          <SectionTitle isHeader weight={700}>
            Buscar modelos
          </SectionTitle>
          <Menu
            mode="inline"
        ...
        <Bar />
        <Content>
          <SectionTitle isHeader>Modelos de templates</SectionTitle>
          <NewTemplateContent>
            <IconTemplate isNew>
              <PlusIcon />
            </IconTemplate>
          ...
          <SectionTitle>Meus templates</SectionTitle>
          <MyTemplates>
            {templates?.length ? (
              templates.map((template) => {
                return (
                  <Item key={template.id}>
                    ...
                    {template.name}
                    <Information>{template.total} produtos</Information>
                  </Item>
                );
              })
            ) : (
          ...
          <SectionTitle>Mais templates</SectionTitle>
          <Contents>
            {templateContents.map((item) => (
              <NewTemplateContent isItem key={item.id}>
                <IconTemplate>
                  <Flag />
                </IconTemplate>
                <TemplateLabel>
